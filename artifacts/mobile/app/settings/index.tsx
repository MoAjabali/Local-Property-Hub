...
import {
  ...
  TextInput, Modal, KeyboardAvoidingView, Platform, Alert, Switch,
} from 'react-native';
...
const S = '#10B981'; const D = '#EF4444'; const A = '#C9A84C';
...
export default function SettingsScreen() {
  ...
  const handleAddCurrency = async () => {
    ...
    Alert.alert('تم', 'تمت إضافة العملة بنجاح');
  };

  const handleAddRate = async () => {
    const rate = parseFloat(rateValue);
    if (!rateFrom || !rateTo || isNaN(rate) || rate <= 0) {
      Alert.alert('خطأ', 'أدخل بيانات صحيحة');
      return;
    }
    setSaving(true);
    ...
    <View style={[s.container, { backgroundColor: BG }]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.card}>
          ...
          <TextInput
            style={s.input}
            value={ownerName}
            onChangeText={v => { setOwnerName(v); setOwnerSaved(false); }}
        ...
        {currencies.length > 1 && (
          <View style={s.card}>
            ...
            {exchangeRates.length === 0 ? (
              <Text style={s.emptyTxt}>لا توجد أسعار صرف</Text>
            ) : (
              exchangeRates.slice(-10).reverse().map(r => (
                <View key={r.id} style={s.rateRow}>
                  <Text style={s.rateVal}>{r.rate.toFixed(4)}</Text>
                  <Text style={s.rateDesc}>{getCurrencyName(r.fromCurrencyId)} ← {getCurrencyName(r.toCurrencyId)}</Text>
                </View>
              ))
            )}
          </View>
      ...
      <Modal visible={showCurrencyForm} transparent animationType="slide" onRequestClose={() => setShowCurrencyForm(false)}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.sheet}>
          ...
          <Text style={s.fl}>رمز العملة (مختصر) *</Text>
          <TextInput style={s.input} value={curSymbol} onChangeText={setCurSymbol} placeholder="مثال: $" textAlign="right" />
          <TouchableOpacity
            style={[s.submit, (!curCode.trim() || !curName.trim() || !curSymbol.trim()) && { opacity: 0.5 }]}
            onPress={handleAddCurrency}
            disabled={!curCode.trim() || !curName.trim() || !curSymbol.trim() || saving}
          >
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.sheet}>
          ...
          <View style={s.curPickers}>
            {currencies.map(c => (
              <TouchableOpacity
                key={c.id}
                style={[s.curPill, rateFrom === c.id && s.curPillActive]}
                onPress={() => setRateFrom(c.id)}
              >
          ...
          <View style={s.curPickers}>
            {currencies.map(c => (
              <TouchableOpacity
                key={c.id}
                style={[s.curPill, rateTo === c.id && s.curPillActive]}
                onPress={() => setRateTo(c.id)}
              >
...
const s = StyleSheet.create({
  ...
  rateDesc: { fontSize: 13, color: T, textAlign: 'right', flex: 1 },
